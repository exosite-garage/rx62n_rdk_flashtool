...
{
    rdk_meta_init();
    write_data_flash(writeBuffer, numBytes, writeAddress);
}
unsigned char rdk_mac_write(unsigned char * mac_buffer) //, unsigned short srcBytes, unsigned char * location)
{
  rdk_mac_block mac_block;
  unsigned long offset=242;

  memcpy((char *)((unsigned long)(&mac_block)),          (char *)RDK_MAC_BLOCK_LOCATION, sizeof(mac_block));  // read our nv struct into RAM
  memcpy((char *)((unsigned long)(&mac_block) + offset), (char *)mac_buffer, 6);  // replace specific member info
  erase_data_flash(15,15); //just erase block 15, the MAC block location

  write_data_flash((unsigned char *)&mac_block, sizeof(mac_block), (unsigned long)RDK_MAC_BLOCK_LOCATION);
